! function (e) {
   ...
   AFRAME.registerComponent("environment", {
      ...
      init: function () {
         this.environmentData = {}, this.STAGE_SIZE = 200, this.assets = {
            arches: [{
               type: "mesh",
      ...
      getFogColor: function (e, t) {
         ...
         return r.multiplyScalar(.9), r.lerp(new THREE.Color(this.data.groundColor), .3), "#" + r.getHexString()
      },
      update: function (e) {
         var t;
         ...
         if (i.normalize(), this.sunlight) {
            if (this.sunlight.setAttribute("position", this.environmentData.lightPosition), "atmosphere" != r) {
               ...
               o.r = (o.r + 1) / 2, o.g = (o.g + 1) / 2, o.b = (o.b + 1) / 2, this.hemilight.setAttribute("light", {
            ...
            } else this.sunlight.setAttribute("light", {
               intensity: .1 + .5 * i.y
            }), this.hemilight.setAttribute("light", {
               intensity: .1 + .5 * i.y
            });
         ...
         if (r !== t.skyType || this.environmentData.skyColor != t.skyColor || this.environmentData.horizonColor != t.horizonColor) {
            ...
            } [r], this.stars && this.stars.setAttribute("visible", "atmosphere" == r), "color" == r ? (n.color = this.environmentData.skyColor, n.fog = !1) : "gradient" == r && (n.topColor = this.environmentData.skyColor, n.bottomColor = this.environmentData.horizonColor), this.sky.setAttribute("material", n)
         }
         "atmosphere" == r && (this.sky.setAttribute("material", {
            sunPosition: i
         }), this.setStars(2e3 * (1 - Math.max(0, 8 * (i.y + .08))))), this.environmentData.fog > 0 ? this.el.sceneEl.setAttribute("fog", {
         ...
         var s = !this.groundGeometry || this.environmentData.seed != t.seed || this.environmentData.ground != t.ground || this.environmentData.playArea != t.playArea || this.environmentData.flatShading != t.flatShading;
         (s || this.environmentData.groundColor != t.groundColor || this.environmentData.groundColor2 != t.groundColor2 || this.environmentData.groundYScale != t.groundYScale || this.environmentData.groundTexture != t.groundTexture || this.environmentData.gridColor != t.gridColor || this.environmentData.grid != t.grid) && (this.updateGround(s), this.hemilight && this.hemilight.setAttribute("light", {
      ...
      dumpParametersDiff: function () {
         ...
         for (var i in this.schema)
            if (!("preset" == i || r && void 0 === r[i])) {
               ...
               if ("vec3" == s) {
                  var a = o;
                  "string" == typeof o && (o = o.split(" "), a = {
                     x: o[0],
                     y: o[1],
                     z: o[2]
                  }), e(a.x) == e(n.x) && e(a.y) == e(n.y) && e(a.z) == e(n.z) || t.push(i + ": " + e(n.x) + " " + e(n.y) + " " + e(n.z))
               } else o != n && ("number" == this.schema[i].type && (n = e(n)), t.push(i + ": " + n))
      ...
      updateGround: function (e) {
         ...
         if (e) {
            ...
            for (var o = new r, n = this.groundGeometry.attributes.position.array, s = n.length, a = 10, l = a / t, d = 0, h = 0, g = 2; g < s; g += 3)
               if ("flat" != this.environmentData.ground) {
                  ...
                  switch (this.environmentData.ground) {
                     case "hills":
                        ...
                        break;
                     case "canyon":
                        c = .2 + .8 * o.noise(d, h, 0), c = Math.min(1, 10 * Math.pow(c, 2));
                        break;
                     case "spikes":
                        c = this.random(g) < .02 ? this.random(g + 1) : 0;
                        break;
                     case "noise":
                        c = this.random(g) < .35 ? this.random(g + 1) : 0
                  }
                  c += .1 * this.random(g + 2);
                  var m = 2 * d / a - 1,
                     u = 2 * h / a - 1,
                     f = this.environmentData.playArea;
                  m = Math.max(0, Math.min(1, (Math.abs(m) - (f - .9)) * (1 / f))), u = Math.max(0, Math.min(1, (Math.abs(u) - (f - .9)) * (1 / f))), c *= m > u ? m : u, c < .01 && (c = 0), n[g] = c, d += l, d >= 10 && (d = 0, h += l)
               } else n[g] = 0;
            this.groundGeometry.computeFaceNormals(), this.groundGeometry.computeVertexNormals(), this.groundGeometry.attributes.position.needsUpdate = !0, this.groundGeometry.attributes.normal.needsUpdate = !0
         }
         ...
         var p = 2048,
            v = 20,
            y = this.STAGE_SIZE / v;
         this.groundCanvas && this.groundCanvas.width == p || (this.gridCanvas = document.createElement("canvas"), this.gridCanvas.width = p, this.gridCanvas.height = p, this.gridTexture = new THREE.Texture(this.gridCanvas), this.gridTexture.wrapS = THREE.RepeatWrapping, this.gridTexture.wrapT = THREE.RepeatWrapping, this.gridTexture.repeat.set(y, y), this.groundCanvas = document.createElement("canvas"), this.groundCanvas.width = p, this.groundCanvas.height = p, this.groundTexture = new THREE.Texture(this.groundCanvas), this.groundTexture.wrapS = THREE.RepeatWrapping, this.groundTexture.wrapT = THREE.RepeatWrapping, this.groundTexture.repeat.set(y, y), this.groundMaterialProps = {
            map: this.groundTexture,
            emissive: new THREE.Color(16777215),
            emissiveMap: this.gridTexture
         }, (new THREE.Material).hasOwnProperty("shading") ? this.groundMaterialProps.shading = this.environmentData.flatShading ? THREE.FlatShading : THREE.SmoothShading : this.groundMaterialProps.flatShading = this.environmentData.flatShading, this.groundMaterial = new THREE.MeshLambertMaterial(this.groundMaterialProps));
         var C = this.groundCanvas.getContext("2d"),
            x = this.gridCanvas.getContext("2d");
         if (this.drawTexture(C, p, v), x.fillStyle = "#000000", x.fillRect(0, 0, p, p), this.drawGrid(x, p, v), this.groundTexture.needsUpdate = !0, this.gridTexture.needsUpdate = !0, e) {
      ...
      drawGrid: function (e, t, r) {
         if ("none" != this.environmentData.grid) {
            var i, o, n, s = Math.floor(r / 2),
               a = t / (r / 2);
            switch (e.fillStyle = this.environmentData.gridColor, this.environmentData.grid) {
               case "1x1":
               case "2x2":
                  for ("1x1" == this.environmentData.grid && (s *= 2, a = t / r), i = 0; i < s; i++) n = Math.floor(i * a), e.fillRect(0, n, t, 1), e.fillRect(n, 0, 1, t);
                  break;
               case "crosses":
               ...
               case "dots":
                  for (i = 0; i < s + 1; i++)
                     for (o = 0; o < s + 1; o++) e.beginPath(), e.arc(Math.floor(o * a), Math.floor(i * a), 4, 0, 2 * Math.PI), e.fill();
                  break;
               case "xlines":
         ...
         if ("none" != this.environmentData.groundTexture) switch (this.environmentData.groundTexture) {
            case "checkerboard":
               ...
               var g = Math.floor(i / 2),
                  c = t / (i / 2);
               for (o = 0; o < g + 1; o += 2)
                  for (var m = 0; m < g + 1; m++) e.fillRect(Math.floor((o + m % 2) * c), Math.floor(m * c), Math.floor(c), Math.floor(c));
               break;
            case "squares":
               var u = 16,
                  f = t / u;
               for (s = new THREE.Color(this.environmentData.groundColor), a = new THREE.Color(this.environmentData.groundColor2), o = 0; o < u * u; o++) n = this.random(o + 3) > .5 ? s.clone() : a.clone(), n.addScalar(.1 * this.random(o + 3) - .05), e.fillStyle = "#" + n.getHexString(), e.fillRect(o % u * f, Math.floor(o / u) * f, f, f);
               break;
            case "noise":
               d = e.getImageData(0, 0, t, t), l = d.data, s = new THREE.Color(this.environmentData.groundColor), a = new THREE.Color(this.environmentData.groundColor2);
               var p = new THREE.Color(a.r - s.r, a.g - s.g, a.b - s.b),
                  v = new r;
               for (o = 0, m = 0, h = l.length; o < h; o += 4, m++) {
                  var y = v.noise(m % t / t * 3, m / t / t * 3, 0);
                  l[o + 0] = Math.floor(255 * (s.r + p.r * y)), l[o + 1] = Math.floor(255 * (s.g + p.g * y)), l[o + 2] = Math.floor(255 * (s.b + p.b * y))
               }
            ...
            case "walkernoise":
               ...
               E.fillStyle = this.environmentData.groundColor, E.fillRect(0, 0, C, C), d = E.getImageData(0, 0, C, C), l = d.data, s = new THREE.Color(this.environmentData.groundColor), a = new THREE.Color(this.environmentData.groundColor2);
               var b = [],
                  T = 1e3;
               for (o = 0; o < T; o++) n = s.clone().lerp(a, Math.random()), b.push({
                  x: Math.random() * C,
                  y: Math.random() * C,
      ...
      getAssetGeometry: function (e) {
         function t(e, t) {
            for (var r = (new THREE.Vector3, e.attributes.position.array), i = r.length, o = 0; o < i; o += 3) r[o] = (s.random(o) - .5) * t, r[o + 1] = (s.random(o + i) - .5) * t, r[o + 2] = (s.random(o + 2 * i) - .5) * t;
         ...
         for (var r, i, o, n = [], s = this, a = 0; a < e.length; a++)
            if ("lathe" == e[a].type) {
               var l = -99999,
                  d = [];
               for (o = e[a].vertices, r = 0; r < o.length; r += 2) d.push(new THREE.Vector2(o[r], o[r + 1])), o[r + 1] > l && (l = o[r + 1]);
               i = new THREE.LatheGeometry(d, e[a].segments || 8), i.applyMatrix4((new THREE.Matrix4).makeRotationFromEuler(new THREE.Euler(-Math.PI, 0, 0))), i.applyMatrix4((new THREE.Matrix4).makeTranslation(0, l, 0)), i = i.toNonIndexed(), n.push(i)
            } else if ("extrude" == e[a].type) {
            var h = new THREE.Shape;
            for (o = e[a].vertices, r = 0; r < o.length; r += 2) 0 == r ? h.moveTo(o[r], o[r + 1]) : h.lineTo(o[r], o[r + 1]);
            i = new THREE.ExtrudeGeometry(h, {
               amount: 1,
               bevelEnabled: !1
            }), i.applyMatrix((new THREE.Matrix4).makeRotationFromEuler(new THREE.Euler(-Math.PI / 2, 0, 0))), e[a].noise && t(i, e[a].noise), n.push(i)
         } else if ("mesh" == e[a].type) {
            i = new THREE.BufferGeometry, o = e[a].vertices;
            var g = e[a].faces,
               c = new Float32Array(o);
            if (i.setIndex(g), i.setAttribute("position", new THREE.BufferAttribute(c, 3)), e[a].mirror) {
               var m = i.clone();
               m.applyMatrix4((new THREE.Matrix4).makeRotationFromEuler(new THREE.Euler(0, Math.PI, 0))), i = THREE.BufferGeometryUtils.mergeBufferGeometries([i, m])
            }
            e[a].noise && t(i, e[a].noise), i = i.toNonIndexed(), i.computeVertexNormals(), n.push(i)
         }
         return n
      },
      updateDressing: function () {
         ...
         if (this.dressing.setAttribute("visible", "none" != this.environmentData.dressing), "none" != this.environmentData.dressing) {
            ...
            switch (this.environmentData.dressing) {
               case "cubes":
                  r = [new THREE.BoxGeometry(1, 1, 1)], r[0].applyMatrix((new THREE.Matrix4).makeTranslation(0, .5, 0));
                  break;
               case "pyramids":
                  r = [new THREE.ConeGeometry(1, 1, 4, 1, !0)], r[0].applyMatrix((new THREE.Matrix4).makeTranslation(0, .5, 0));
                  break;
               case "cylinders":
                  r = [new THREE.CylinderGeometry(.5, .5, 1, 8, 1, !0)], r[0].applyMatrix((new THREE.Matrix4).makeTranslation(0, .5, 0));
                  break;
               default:
            ...
            for (var i = 0, o = 88343; i < this.environmentData.dressingAmount; i++, o++) {
               var n, s = (r[Math.floor(this.random(33 + i) * r.length)].clone, r[Math.floor(this.random(33 + i) * r.length)].clone()),
               ...
               g.compose(new THREE.Vector3(Math.cos(h) * n, 0, Math.sin(h) * n), (new THREE.Quaternion).setFromAxisAngle(new THREE.Vector3(0, 1, 0), (this.random(o + 5) - .5) * l.length() * Math.PI * 2), new THREE.Vector3(a + (m ? c : this.random(o + 6)) * l.x, a + (m ? c : this.random(o + 7)) * l.y, a + (m ? c : this.random(o + 8)) * l.z)), s.applyMatrix4(g), t.push(s)
            }
            var u = THREE.BufferGeometryUtils.mergeBufferGeometries(t);
            u.attributes.position.needsUpdate = !0;
            var f = new THREE.MeshLambertMaterial({
                  color: new THREE.Color(this.environmentData.dressingColor)
               }),
               p = new THREE.Mesh(u, f);
            e.add(p), this.dressing.setObject3D("mesh", e)
         }
      },
      createStars: function () {
         for (var e = 2e3, t = new THREE.BufferGeometry, r = new Float32Array(3 * e), i = this.STAGE_SIZE - 1, o = new THREE.Vector3, n = 0; n < r.length; n += 3) o.set(this.random(n + 23) - .5, this.random(n + 24), this.random(n + 25) - .5), o.normalize(), o.multiplyScalar(i), r[n] = o.x, r[n + 1] = o.y, r[n + 2] = o.z;
      ...
      setStars: function (e) {
         this.stars || (this.stars = document.createElement("a-entity"), this.stars.id = "stars", this.createStars(), this.el.appendChild(this.stars)), e = Math.floor(Math.min(2e3, Math.max(0, e))), this.stars.getObject3D("mesh").geometry.setDrawRange(0, e)
      }
